...
def plotGeneratedImagesPatchCamelyon(epoch, examples=100, dim=(10, 10), figsize=(10, 10)):
    noise = [np.random.normal(0, 1, size=[100, latent_dim]), np.random.randint(0, 2, 100)]
...
def get_discriminator_histopathology(
        in_shape=(32, 32, 3), n_classes=2):
    # label input
    in_label = Input(shape=(1,))
    # embedding
    l_lay = layers.Embedding(n_classes, 50)(in_label)
    n_nodes = in_shape[0] * in_shape[1]
    l_lay = Dense(n_nodes)(l_lay)
    l_lay = Reshape((in_shape[0], in_shape[1], 1))(l_lay)
    # image input
    inputs = Input(shape=in_shape)
    # concatenation
    x = layers.Concatenate()([inputs, l_lay])
    ...
    discriminator = Model(inputs=[inputs, in_label], outputs=outputs)
    return discriminator


def get_generator_histopathology(latent_dim=100, n_classes=2):
    # label input
    in_label = Input(shape=(1,))
    l_lay = layers.Embedding(n_classes, 50)(in_label)
    n_nodes = 4 * 4
    l_lay = Dense(n_nodes)(l_lay)
    l_lay = Reshape((4, 4, 1))(l_lay)
    inputs = Input(shape=(latent_dim,))
    x = Dense(128 * 4 * 4, kernel_initializer=keras.initializers.RandomNormal(stddev=0.02))(inputs)
    x = LeakyReLU(0.2)(x)
    x = Reshape((4, 4, 128))(x)
    x = layers.Concatenate()([x, l_lay])
    ...
    generator = Model(inputs=[inputs, in_label], outputs=outputs)
    return generator
...
generator = get_generator_histopathology(latent_dim=latent_dim)
...
z, gen_y = generator.input
x = generator([z, gen_y])
D_G_z = discriminator([x, gen_y])
gan = Model(inputs=[z, gen_y], outputs=D_G_z)
...
for e in range(epochs):
    for b in range(batch_count):
        ...
        # Get a random set of input noise and images
        noise = [np.random.normal(0, 1, size=[batch_size, latent_dim]), np.random.randint(0, 2, batch_size)]
        labels_noise = noise[1]
        # image_batch = X_train[np.random.randint(0, X_train.shape[0], size=batch_size)]
        image_batch, labels_real = next(train_gen)
        image_batch = tf.image.central_crop(image_batch, 0.333)
        ...
        # concatenate the labels for fake and real samples
        labels = np.concatenate([labels_real, labels_noise])

        # Train discriminator with this batch of samples
        discriminator.trainable = True
        try:
            d_loss = discriminator.train_on_batch([X, labels], y_dis)
        except ValueError:
    ...
    if e % 5 == 0:
        ...
        # plotImagesPatchCamelyon(generatedImages,
        #                         title='Epoch {}'.format(e))  # map pixel values to the [0, 1] range for plotting
        plotGeneratedImagesPatchCamelyon(e)
        display.display(plt.gcf())
        display.clear_output(wait=True)
        time.sleep(0.001)
        saveModels(e)
