...
disc = keras.models.load_model('gan_discriminator_epoch_lower_lr145.h5',
...
def plot_images_ordered(images, next_weights, dim=[8, 8], figsize=[8, 8]):
    order = get_order(next_weights)
...
def plot_images(images, dim=[5, 5], figsize=[5, 5]):
    plt.figure(figsize=figsize)
...
def get_weights(model, layer):
    """
    returns the weights of the specified convolutional layer and the next convolutional layer
    """
    ...
    # second_layer_weights = second_layer_weights[0,0][:,0]
...
def visualize_first_layer_weights():
    first_layer_weights, second_layer_weights = get_weights(disc, 0)
    images = preprocess_images(first_layer_weights)
    plot_images_ordered(images, second_layer_weights)
    first_layer_weights, second_layer_weights = get_weights(classifier, 0)
    images = preprocess_images(first_layer_weights)
    plot_images_ordered(images, second_layer_weights)
...
def visualize_layer_weights(layer, n_images, model):
    ...
    plot_images(images, dim=dim, figsize=figsize)
disc.summary()
classifier.summary()
