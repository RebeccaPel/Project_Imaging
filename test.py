...
def define_gan(g_model, d_model):
    ...
    return model
def load_pcam_data():
    directory = r"C:\Users\justi\PycharmProjects\pythonProject\train+val\train"
    ...
    trainX = dataset[0][0]
    trainy = dataset[0][1]
    for test in itertools.islice(dataset, 0, 100):
        trainX = np.append(trainX, test[0], axis=0)
        trainy = np.append(trainy, test[1])
    return (trainX, trainy), (1,2)
# load the images
def load_real_samples():
    # load dataset
    (trainX, trainy), (_, _) = load_pcam_data()
